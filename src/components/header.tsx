        ...
        <div className="flex items-center gap-2 md:hidden">
          <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
             ...
             <SheetContent
                forceMount
                data-transition-all="true" data-duration-300="true"
                side="right"
             ...
             >
              ...
              <nav className="flex flex-col justify-center items-center mt-6">
                {mobileNavItems.map((item, index) => (
                  ...
                  href={item.href} 
                  onClick={() => setOpen(false)}
                  className={`block w-full px-4 py-3 text-xl font-medium text-foreground hover:bg-accent/10 dark:hover:bg-accent/20 hover:text-accent dark:hover:text-accent transition-all duration-200 ${index === mobileNavItems.length - 1 ? 'border-b border-border/20' : ''}`} >
                    // Close the sheet here if needed
                   }}>
