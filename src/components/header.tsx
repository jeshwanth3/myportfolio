    ...
    <header className={cn(
      ...
      <div className="container flex h-20 items-center justify-between px-4 md:px-6 max-w-7xl mx-auto">
        ...
        <Link href="/" className="flex items-center space-x-3 group" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
          <span className={cn(
              "text-2xl sm:text-3xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-primary via-accent to-primary animate-gradient-text-slow",
        ...
        <nav className="hidden md:flex items-center gap-1 lg:gap-2">
          ...
          <Link href="#contact" passHref>
             <Button
               variant="outline"
               size="sm"
               className="ml-4 px-4 py-2 h-9 shadow-sm hover:shadow-md transition-all hover:scale-105 transform duration-200 border-primary/40 hover:border-primary/70 text-primary hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent dark:hover:text-accent-foreground"
             >
        ...
        <div className="flex items-center gap-2 md:hidden">
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent
                side="right"
                className="w-[85vw] max-w-[380px] bg-background/90 border-l border-border/30 backdrop-blur-xl p-0 flex flex-col glassmorphism-heavy"
                ...
                {navItems.map((item) => (
                 ...
                 <Link
                    ...
                    onClick={(e) => { e.preventDefault(); handleMobileLinkClick('#contact'); }}
                     className="block w-full px-4 py-3 text-lg font-medium text-foreground hover:bg-accent/10 dark:hover:bg-accent/15 hover:text-accent dark:hover:text-primary rounded-md transition-all duration-200 text-center mt-4 border-t border-border/20 pt-6"
                  >
