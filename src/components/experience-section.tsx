      ...
      <Accordion type="single" collapsible className="w-full space-y-4">
        {experiences.map((exp) => (
          <AccordionItem key={exp.id} value={exp.id} className="border border-border/30 dark:border-border/50 rounded-lg bg-card/90 dark:bg-secondary/60 mac-shadow transition-all duration-300 ease-out overflow-hidden hover:border-primary/40 dark:hover:border-accent/50 transform hover:-translate-y-0.5"> {/* Updated hover border to primary/accent */}
            <AccordionTrigger className="p-4 md:p-6 text-left hover:no-underline hover:bg-primary/5 dark:hover:bg-accent/10 transition-colors data-[state=open]:bg-primary/10 dark:data-[state=open]:bg-accent/15 hover:text-primary dark:hover:text-accent dark:data-[state=open]:text-accent"> {/* Use primary/accent color for hover and open states */}
               <div className="flex items-start gap-4 w-full">
                  {/* Ensure icon background uses theme color */}
                  <div className="mt-1 shrink-0 bg-primary/10 dark:bg-accent/20 p-2 rounded-full">
                      {exp.icon}
                  </div>
