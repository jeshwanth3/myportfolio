...
export function SummarySection() {
  return (
    // Adjusted background and padding for better flow
    <SectionWrapper id="summary" className="bg-gradient-to-b from-background via-card/5 to-background/90 pt-24 md:pt-32 lg:pt-36">
      <div className="grid grid-cols-1 md:grid-cols-5 gap-10 lg:gap-16 items-center">
        ...
        <div className="md:col-span-3 space-y-5 md:space-y-6 text-center md:text-left">
          <p className="text-lg sm:text-xl text-muted-foreground md:text-2xl lg:text-2xl max-w-xl mx-auto md:mx-0 font-light"> {/* Slightly adjusted size/weight */}
             Hello, I&apos;m
          </p>
           ...
           <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight md:text-5xl lg:text-5xl text-primary animate-glow"> {/* Adjusted size */}
             Product Manager
           </h2>
          {/* Summary Text - Improved readability */}
          <p className="text-lg sm:text-xl text-muted-foreground md:text-xl max-w-2xl mx-auto md:mx-0 leading-relaxed"> {/* Adjusted size */}
            {professionalSummary}
          </p>
          ...
          <div className="flex flex-col sm:flex-row justify-center md:justify-start gap-4 pt-6 md:pt-8"> {/* Increased top padding */}
            <Link href="#contact" passHref>
              <Button size="lg" className="shadow-md hover:shadow-lg transition-shadow hover:scale-[1.03] transform duration-300 w-full sm:w-auto button-glow"> {/* Added glow class */}
