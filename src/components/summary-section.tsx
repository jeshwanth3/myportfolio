    ...
    <SectionWrapper id="summary" className="bg-gradient-to-b from-background via-card/5 to-background/90 pt-24 md:pt-32 lg:pt-36">
      <div className="grid grid-cols-1 md:grid-cols-5 gap-10 lg:gap-16 items-center">
        <div className="md:col-span-3 space-y-5 md:space-y-6 text-center md:text-left">
          <p className="text-lg sm:text-xl text-muted-foreground md:text-2xl lg:text-2xl max-w-xl mx-auto md:mx-0 font-light">
             Hello, I&apos;m
          </p>
           <h1 className="text-4xl sm:text-5xl font-bold tracking-tight md:text-6xl lg:text-7xl text-foreground leading-tight">
            {yourName}
          </h1>
           <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight md:text-5xl lg:text-5xl text-primary animate-glow">
             Product Manager
           </h2>
          <p className="text-lg sm:text-xl text-muted-foreground md:text-xl max-w-2xl mx-auto md:mx-0 leading-relaxed">
            {professionalSummary}
          </p>
          <div className="flex flex-col sm:flex-row justify-center md:justify-start gap-4 pt-6 md:pt-8">
            <Link href="#contact" passHref>
              <Button size="lg" className="shadow-md hover:shadow-lg transition-shadow hover:scale-[1.03] transform duration-300 w-full sm:w-auto button-glow">
                Contact Me <ArrowRight className="ml-2 h-5 w-5" /> {/* Adjusted icon size */}
              </Button>
            </Link>
        ...
        <div className="md:col-span-2 flex justify-center items-center md:justify-end order-first md:order-last">
          <Image
            ...
            className="rounded-full mac-shadow border-4 border-primary/30 dark:border-primary/40 object-cover aspect-square w-[65vw] max-w-[260px] h-auto sm:max-w-[320px] md:max-w-[380px]"
            priority
          />
        </div>
      </div>
