...
export function Experience() {
  return (
    <SectionWrapper id="experience" className="bg-background py-24">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <SectionTitle>Professional Experience</SectionTitle>
        <p className="mt-4 max-w-3xl text-muted-foreground" style={{ lineHeight: '1.85' }}>
          Demonstrated business analysis leadership across healthcare, enterprise IT, and supply chain domains with a strong focus on AI-enabled workflows and actionable requirements.
        </p>

        <div className="mt-10 space-y-8">
          {experiences.map((experience) => (
            <Card key={experience.id} className="overflow-hidden rounded-[2rem] border border-border/50 bg-card/80 shadow-lg">
              <CardHeader className="flex flex-col gap-4 border-b border-border/50 p-6 md:flex-row md:items-center md:justify-between">
                <div>
                  <CardTitle className="text-xl font-semibold text-foreground">{experience.title}</CardTitle>
                  <p className="text-sm text-muted-foreground">{experience.company}</p>
                </div>
                <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                  <span className="inline-flex items-center gap-2 rounded-2xl bg-background/70 px-3 py-2 border border-border/40">
                    <MapPin className="h-4 w-4 text-primary" /> {experience.location}
                  </span>
                  <span className="inline-flex items-center gap-2 rounded-2xl bg-background/70 px-3 py-2 border border-border/40">
                    <Calendar className="h-4 w-4 text-primary" /> {experience.duration}
                  </span>
                </div>
              </CardHeader>
